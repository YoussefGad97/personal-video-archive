...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ video, isOpen, onClose, onSelectVideo }) => {
  ...
  useEffect(() => {
    if (isOpen && video) {
      const fetchSuggestedVideos = async () => {
        ...
        const suggested = allVideos.filter(v => 
          v.id !== video.id &&
          v.playlists.some(p => video.playlists.includes(p))
        );
  ...
  }, [isOpen, volume, isPlaying, isMuted, isFullscreen, isPictureInPicture, togglePlay, skipTime, toggleMute, enterFullscreen, togglePictureInPicture, restartVideo, exitFullscreen, handleDialogClose]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleDialogClose}>
      <DialogContent className="sm:max-w-[90vw] md:max-w-[80vw] lg:max-w-[95vw] xl:max-w-[80vw] p-0 bg-background/95 backdrop-blur-md overflow-hidden animate-scale-in">
        <div className="flex flex-col lg:flex-row">
          ...
          {suggestedVideos.length > 0 && (
            <div className="w-full lg:w-96 bg-background/80 p-4 overflow-y-auto max-h-[80vh]">
              ...
              <div className="space-y-4">
                {suggestedVideos.map(suggestedVideo => (
                  <div 
                    ...
                    onClick={() => onSelectVideo(suggestedVideo)}
                  >
                    <img 
                    ...
                    />
                    <div>
                      <h5 className="text-sm font-medium line-clamp-2">{suggestedVideo.title}</h5>
                      <p className="text-xs text-muted-foreground">{formatDuration(suggestedVideo.duration)}</p>
                    </div>
                  </div>
                ))}
              </div>
